...
const es: UiStrings = {
  ...
  changelog: {
    ...
    entries: [
      { date: '2026-08-04', title: 'Cuenta opcional: sincroniza tus mazos e inventario', summary: 'Inicia sesión con una passkey para sincronizar tus mazos e inventario entre dispositivos, cifrado de extremo a extremo con tu código de recuperación para que SchreckNet no pueda leerlos. Todo sigue funcionando sin cuenta.', items: ['Sin contraseña ni correo electrónico — inicia sesión con una passkey (Face ID, huella dactilar o llave de seguridad), respaldada por tu dispositivo o gestor de contraseñas.', 'La sincronización se cifra en el propio dispositivo; el servidor solo almacena texto cifrado, y las ediciones conflictivas de dos dispositivos se señalan en vez de sobrescribirse en silencio.', 'Los tokens de API permiten usar los endpoints de sincronización MCP/REST desde otras herramientas, limitados deliberadamente solo a la sincronización — no pueden gestionar tus passkeys ni eliminar tu cuenta.', 'Elimina tu cuenta cuando quieras; es una eliminación real y permanente, y tus mazos locales no se ven afectados en ningún caso.'] },
