...
const fr: UiStrings = {
  ...
  account: {
    ...
    loginButton: 'Se connecter avec une passkey',
    loggingIn: 'Connexion…',
    switchToLogin: 'Déjà un compte ? Se connecter',
    switchToRegister: "Pas encore de compte ? En créer un",
    recoveryInstead: 'Toutes vos passkeys perdues ? Utilisez votre code de récupération',
    recoveryCodeLabel: 'Code de récupération',
    recoverButton: 'Récupérer le compte',
    recovering: 'Récupération…',
    backToLogin: '← Retour à la connexion',
    signedInAs: (name) => `Connecté·e en tant que ${name}`,
    memberSince: (date) => `Membre depuis le ${date}`,
    logout: 'Se déconnecter',
    loggingOut: 'Déconnexion…',
    dataSafetyTitle: "Que reste-t-il après effacement des données du navigateur ?",
    dataSafetyPasskey: "Votre passkey — elle vit dans votre appareil ou votre gestionnaire de mots de passe, pas dans le stockage de ce site. Non affectée.",
    dataSafetySession: "Votre connexion — vous serez déconnecté·e. Reconnectez-vous, c'est tout.",
    dataSafetyDecks: "Vos decks et inventaire locaux — détruits, exactement comme sans compte. Faites une sauvegarde depuis « Données et sauvegarde ».",
    passkeyManagerNote: "La meilleure protection : ajoutez une deuxième passkey sur un autre appareil avant d'en avoir besoin.",
    passkeysTitle: 'Vos passkeys',
    passkeysNote: "Une ligne par appareil ou gestionnaire de mots de passe pouvant vous connecter. Ajoutez-en un autre avant de perdre l'accès à celui-ci.",
    loadingPasskeys: 'Chargement des passkeys…',
    addPasskeyButton: 'Ajouter une passkey',
    addingPasskey: 'Ajout…',
    lastPasskeyBadge: 'seule passkey',
    unnamedPasskey: 'Passkey sans nom',
    renamePasskey: 'Renommer',
    removePasskey: 'Supprimer',
    removing: 'Suppression…',
    nicknamePrompt: 'Étiquette pour cette passkey (ex. « iPhone », « ordinateur du travail »)',
    recoveryCodeTitle: 'Enregistrez votre code de récupération',
    recoveryCodeIntro: "C'est le seul moyen de retrouver votre compte si vous perdez toutes vos passkeys. Il n'est affiché qu'une seule fois et n'est plus jamais récupérable — enregistrez-le en lieu sûr, par exemple dans un gestionnaire de mots de passe.",
    recoveryCodeSavedConfirm: "J'ai enregistré mon code de récupération",
    recoveryCodeContinue: 'Continuer',
    newRecoveryCodeTitle: 'Votre code de récupération a changé',
    newRecoveryCodeIntro: "Utiliser un code de récupération le rend invalide. Voici le nouveau — enregistrez-le de la même façon.",
    unsupportedBrowser: "Ce navigateur ne prend pas en charge les passkeys, les comptes sont donc indisponibles ici. Tout le reste sur SchreckNet continue de fonctionner.",
    error: (message) => `Un problème est survenu : ${message}`,
  },
  settings: {
  ...
  changelog: {
    ...
    entries: [
      { date: '2026-08-04', title: 'Les imports de deck JOL fonctionnent enfin bien', summary: "Coller une liste de deck exportée depuis JOL résout maintenant chaque carte correctement, au lieu de laisser un « x » collé au nom de la carte.", items: ["Le format propre à JOL « 4 x Nom de carte » (espace avant le x) est maintenant analysé comme le « 4x Nom de carte » de Lackey — vérifié sur un export JOL réel.", "L'import de texte pour les decks et l'inventaire utilisent le même correctif, rien à revérifier deux fois."] },
