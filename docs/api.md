...
| `search_library` | **Live.** Text scope + regex mode (`text_regex`), exact card type, clan/path and discipline requirements, VDB discipline logic (`discipline_logic=all\|any\|none\|only`, plus `include_no_discipline`), derived vampire-capacity requirements (`capacity_requirement` + `capacity_requirement_mode=at_most\|at_least`), independent blood/pool cost comparisons (`at_most`, `exact`, `at_least`), the same V5 set age/printing modes, precon, and artist. Remaining filters: sect/title requirements and traits |
