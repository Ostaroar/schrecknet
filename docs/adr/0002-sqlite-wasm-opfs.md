...
- Server uses rusqlite (WAL) for the same card DB plus `app.sqlite` (accounts, sync,
  PDA). One migration set shared browser/server.
...
- **Server-side Postgres now**: operational weight without need; revisit if PDA/TWD
  write load ever demands it (schema avoids SQLite-only features where cheap).
